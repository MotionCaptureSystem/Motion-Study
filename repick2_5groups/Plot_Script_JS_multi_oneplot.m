...
%             end
            ...
            if tDof(dof)
                ...
                for ss = 1:nstruct
                    legend_handles(length(legend_handles)+1) = plot(t,eststruct(ss).ukf.X(dof+dof_prev,:),linetypes{ss},'Color','k');
                    hold on
                end
                axis tight
                grid on
...
%                 if dof == 1
%                     title([options.dof_names{dof_prev+dof},' Displacements',],options.plot.fig_txt_props{:})
%                 end
%                 if dof == nDof
...
%                 axis tight
                ...
                clear title xlabel ylabel
                for ss = 1:nstruct
                    legend_handles(length(legend_handles)+1) = plot(t,180/pi*eststruct(ss).ukf.X(dof+dof_prev,:),linetypes{ss},'Color','k');
                    hold on
                end
                grid on
                axis tight
