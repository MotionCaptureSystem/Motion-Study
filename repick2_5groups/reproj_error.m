function reproj_error(camstruct, inertstruct, options, linestyle)
%proj error flight 3 progrram flow 
%% Import Plot Settings
nstruct = length(inertstruct);
fs_c = options.fs_c;
...
for ss = 1:nstruct
    ...
    for pp = pts
        cnt = cnt+1;
%         if ss~=3
%             X_ukf{ss}(3*(cnt-1)+1:3*cnt,:)  = inertstruct(ss).ukf.Features(3*(pp-1)+1:3*pp,3:end);
%         else
            X_ukf{ss}(3*(cnt-1)+1:3*cnt,:)  = inertstruct(ss).ukf.Features(3*(cnt-1)+1:3*cnt,3:end);
        %end
    end

    y_k_ukf{ss}         = zeros(2*ncam*npts,nsteps);
    lambdas_ukf{ss}     = zeros(npts,nsteps,ncam);
    meas_dist{ss}       = zeros(2*ncam*npts,nsteps);

    %compile measurements into a matrix
    meas{ss} = zeros(ncam*npts*2,nsteps);
    for c = 1:ncam
        ...
        for pp = 1:npts
            if options.plot.pts_orig(pp)<=size(camstruct(c).pts_sync,3)
                meas{ss}((c-1)*2*npts+2*(pp-1)+1:(c-1)*2*npts+2*pp,:) = camstruct(c).pts_sync(:,t,options.plot.pts_orig(pp));
            else
                meas{ss}((c-1)*2*npts+2*(pp-1)+1:(c-1)*2*npts+2*pp,:) = NaN*ones(2,length(t),1);
            end
        end
    end
    ...
    for kk = 1:nsteps
        meas_dist{ss}(:,kk)                           = CameraDistortion(meas{ss}(:,kk),camstruct);
        [y_k_ukf{ss}(:,kk), lambdas_ukf{ss}(:,kk,:)]  = CamNetDistortion(X_ukf{ss}(:,kk),camstruct);
    end

    %reformat point array
    points_ukf{ss}  = zeros(npts,2,nsteps,ncam);
    points_meas{ss} = zeros(npts,2,nsteps,ncam);
    for c = 1:ncam
        for pp = 1:npts
            for kk = 1:nsteps
                points_meas{ss}(kk,:,pp,c)  = meas_dist{ss}(2*(c-1)*npts+2*(pp-1)+1:2*(c-1)*npts+2*(pp-1)+2,kk)';
                points_ukf{ss}(kk,:,pp,c)   = y_k_ukf{ss}(2*(c-1)*npts+2*(pp-1)+1:2*(c-1)*npts+2*(pp-1)+2,kk)';
            end
        end
    end
end
...
for ss = 1:nstruct
    reproj_error_ukf{ss} = -points_ukf{ss}+points_meas{ss};
end
%% Determine Mean and STD Dev of Reprojection Errors 
%Determine Reprojection Errors in mm

for ss = 1:nstruct
    reproj_error_dist_ukf{ss} = zeros(nsteps,2,npts,ncam);
    for c = 1:ncam
        for pp = 1:npts
            for kk = 1:nsteps
                reproj_error_dist_ukf{ss}(kk,:,pp,c)    = reproj_error_ukf{ss}(kk,:,pp,c);
            end
        end
    end
end
...
for ss = 1:nstruct
    ...
    for c = 1:ncam
        for pp = 1:npts
            %Grab page of points
            col1_ukf = reproj_error_dist_ukf{ss}(plot_start:plot_stop,1,pp,c);
            col2_ukf = reproj_error_dist_ukf{ss}(plot_start:plot_stop,2,pp,c);
        ...
        reproj_mean_ukf{ss}(:,:,c) = mean(AllPts_ukf)';
        reproj_std_ukf{ss}(:,:,c)  = std(AllPts_ukf)';
    ...
    circ_pt_num = 50;
    elipse_ukf{ss} = zeros(circ_pt_num,2,ncam);
    ndev = 1.97;

    cam_hands_ukf = zeros(ncam,1);
    for c = 1:ncam
        for ii = 1:circ_pt_num
            elipse_ukf{ss}(ii,:,c) = (reproj_mean_ukf{ss}(:,:,c) + ndev*reproj_std_ukf{ss}(:,:,c).*[cos(2*pi*ii/circ_pt_num);sin(2*pi*ii/circ_pt_num)])';
        end
    end
end
%% Create Plots
...
for c = 1:ncam
    ...
    if exist(path_name,'file')
        ...
        for pt = 1:npts %[1,2,3,6,7,8,11,12,14,15,17];
            cnt = cnt+1;
            p1 = plot(points_meas{ss}(plot_start:plot_stop,x,pt,c),points_meas{ss}(plot_start:plot_stop,y,pt,c),'+','Color',options.plot.colors(cnt,:), 'LineWidth', 1);
            p2 = [];
            for ss = 1:nstruct
                p2 = [p2, plot(points_ukf{ss}(plot_start:plot_stop,x,pt,c),points_ukf{ss}(plot_start:plot_stop,y,pt,c),linestyle{ss},'Color',options.plot.colors(cnt,:), 'LineWidth', 1)];
            end
            if cnt == 1
        ...
        %[hleg1, hobj1] = legend(legend_handles, 'Image Features', 'Recursive','Location','NorthWest');
    ...
    for pp = 1:npts%[1,2,3,6,7,8,11,12,14,15,17]
        cnt = cnt+1;
        for ss = 1:nstruct
            p2 = [p2, plot3(reproj_error_dist_ukf{ss}(plot_start:plot_stop,1,pp,c),reproj_error_dist_ukf{ss}(plot_start:plot_stop,2,pp,c),zeros(size(reproj_error_dist_ukf{ss}(plot_start:plot_stop,2,pp,c))),marker{ss},'Color',options.plot.colors(cnt,:))];
        end
        if cnt == 1
            legend_handles = p2;
        end
    ...
    p4 = [];
    for ss = 1:nstruct
        p4 = [p4, plot3(elipse_ukf{ss}(:,1,c),elipse_ukf{ss}(:,2,c),10*ones(size(elipse_ukf{ss}(:,2,c))), linestyle{ss}, 'Color', 'k', 'LineWidth', 5)];
    end
    ...
    handle = legend([legend_handles, p4], 'Standard', 'Recursive', '95% Conf Standard', '95% Conf Recursive', 'Location','NorthEast');
