function [camstruct,options] = Init_Bat_20160730_test010_pts(camstruct,options)
%% --------------------------Set Options----------------------------------
%Import Options
...
options.tstop           = 430;                  %Useable timestep will be tstart+1 
options.dt              = 1;
...
options.est.state_init      = repmat([-1.15,0,0]',length(options.pts),1);

%Plot Options
options.plot.pts           = [1];
options.plot.pts_orig      = [92];
