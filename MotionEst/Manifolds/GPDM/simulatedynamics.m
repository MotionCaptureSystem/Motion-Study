function [X_pred, XRand_pred] = simulatedynamics(X, segments, thetad, invKd, simSteps, simStart, modelType, updateKernel)
%%SIMULATEDYNAMICS     -Simulates the latent manifold dynamics learned
%%using GPDMFITFULL.m. 

%X          -Identified latent variables from optimization
%segments   -How many cycles of data are inputted
%thetad     -Hyper params for latent space dynamic regression kernels (beta)
%invKd      -
...
if (modelType(1) < 2)
    ...
    X_pred(1,:) = simStart(:,end-q+1:end);
    XRand_pred(1,:) = simStart(:,end-q+1:end);
%     X_pred(1,:) = simStart(:,1:q);
%     XRand_pred(1,:) = simStart(:,1:q);
end
