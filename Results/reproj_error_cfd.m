function reproj_error_cfd(camstruct, pts, lookuptab, options)
...
for c = 1:ncam
    t = options.tstart-camstruct(c).start_frame+1+floor(camstruct(c).sync_del*fs_c)+2:dt:options.tstop-camstruct(c).start_frame+1+floor(camstruct(c).sync_del*fs_c);
    for pp = 1:npts
        if lookuptab(1,pp)<=size(camstruct(c).pts_sync,3)
            meas((c-1)*2*npts+2*(pp-1)+1:(c-1)*2*npts+2*pp,:) = camstruct(c).pts_sync(:,t,lookuptab(1,pp));
        else
            meas((c-1)*2*npts+2*(pp-1)+1:(c-1)*2*npts+2*pp,:) = NaN*zeros(2,length(t));
        end
    end
end
...
for c = 1:ncam
    ...
    for pp = 1:npts%[1,2,3,6,7,8,11,12,14,15,17]
        cnt = cnt+1;
        p2 = plot(reproj_error_dist_ukf(floor(plot_start/dt):floor(plot_stop/dt),1,pp,c),reproj_error_dist_ukf(floor(plot_start/dt):floor(plot_stop/dt),2,pp,c),'o','Color',colors(cnt,:));
        if cnt == 1
            legend_handles = p2;
        end
    end
