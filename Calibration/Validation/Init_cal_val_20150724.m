...
options.cams            = [301:303,309:310,312,318,320,325,333,334];
options.plotflag        = 0;
options.path            = 'E:\ShandongData2015\batflight_20150724\Calibration_run\Extrinsic(converged31cams10142015)';
options.default_dir     = pwd;

%Bundle Adjustment Options
options.ba                  = 2;                          %3 load previous and run BA, 2 run BA from scratch, 1 load previous, 0 no BA
options.ba_tsteps           = [1:20:100,232,310:20:400,573,662];             %Row vec of tsteps to use in BA
options.ba_pts              = [1];     %Row vec of pts to use in BA

%Stereo Options
options.stereo.pts          = options.pts;
options.stereo.cams         = [301:303,309:310,312,318,320,325,333,334];
options.stereo.cams         = options.cams;
options.stereo.tstart       = 1;
options.stereo.tstop        = 1000;
options.stereo.dt           = 1;
options.stereo.tsteps       = [1:1000];%[1:20:100,232,310:20:400,573,662];%[232,352,440,573,662,740,840,926];%[1:1000];
